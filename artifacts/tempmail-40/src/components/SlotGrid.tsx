    ...
    <div className="flex flex-col bg-card border border-card-border p-4 rounded-lg shadow-sm hover:border-border transition-colors">
      ...
      >
        ...
        <AnimatePresence mode="wait">
          {emailCopied
            ? <motion.div key="check" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }}><Check className="w-4 h-4 text-green-500 shrink-0" /></motion.div>
            : <motion.div key="copy" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }}><Copy className={`w-4 h-4 shrink-0 transition-colors ${slot.email ? 'text-muted-foreground group-hover/copy:text-primary' : 'text-muted-foreground/30'}`} /></motion.div>}
        </AnimatePresence>
      </div>
      ...
      <AnimatePresence>
        {polling && !slot.code && (
          <motion.div key="polling" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }} className="mb-3 overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-cyan-950/20 border border-cyan-800/30">
              ...
              <span className="text-[11px] text-cyan-400 font-mono flex-1">Menunggu kode verifikasi…</span>
              {lastChecked && <span className="text-[10px] text-cyan-700 font-mono shrink-0">{lastChecked.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</span>}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {slot.code && (
          <motion.div key="code" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.25 }}>
            <div className="flex items-center gap-1.5 mb-1.5">
              ...
              <span className="text-xs text-green-400 font-mono font-semibold">Code: {slot.code}</span>
            </div>
            <div onClick={handleCopyCode} className="cursor-pointer rounded-lg border-2 border-green-500/60 bg-green-950/30 hover:bg-green-950/50 transition-colors py-4 flex flex-col items-center justify-center gap-1 select-none">
              <span className="font-mono font-bold text-3xl tracking-[0.3em] text-green-400">{slot.code}</span>
              <span className="text-[10px] text-green-600 font-medium">{codeCopied ? 'Tersalin!' : 'Klik untuk salin'}</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
